...
def update_run_state(root: Path, result: dict, mode: str):
...
def append_analysis(root: Path, result: dict, mode: str):
    p = root / 'analysis.md'
    if not p.exists():
        return
...
def main():
    ...
    try:
        ...
        result['dispatch_mode'] = a.dispatch_mode
        save_json(status_json, result)
        update_run_state(root, result, a.dispatch_mode)
        append_analysis(root, result, a.dispatch_mode)
